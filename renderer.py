...
class Renderer:
    def __init__(self, dx, image_res, up, voxel_edges, exposure=3):
        ...
        self.phiSize = 128
        self.phi = ti.field(dtype=ti.f32, shape=(self.phiSize, self.phiSize, self.phiSize))
    ...
    @ti.kernel
    def getPhi(self):
        for i, j, k in ti.ndrange(self.phiSize, self.phiSize, self.phiSize):
            center = ti.Vector([self.phiSize//2, self.phiSize//2, self.phiSize//2])
            if sumVec((ti.Vector([i, j, k]) - center)**2) < 20**2:
                self.phi[i, j, k] = 1.